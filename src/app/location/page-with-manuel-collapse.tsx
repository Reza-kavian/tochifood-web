...
type BoxHtmlComponentType = {
  heightBox: string;
  setHeightBox: React.Dispatch<React.SetStateAction<string>>;
...
export default function LocationPage() {
  ...
  const [heightBox, setHeightBox] = useState<string>('0px');
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: any) => {
      ...
      if (refForBox.current && !refForBox.current.contains(event.target) && !isEpmtyHeightBox
        ...
        setIsEpmtyHeightBox(true);
        setHeightBox('0px'); // تنظیم مستقیم به 0 برای شروع انیمیشن بستن
      }
      else {
