...
type CollapseAndClickAwayForRaveshErsalCompType = {
    isEpmtyCollapseForRaveshErsal: boolean;
    setIsEpmtyCollapseForRaveshErsal: React.Dispatch<React.SetStateAction<boolean>>;
...
const CollapseAndClickAwayForRaveshErsalComp = ({
    isEpmtyCollapseForRaveshErsal,
    setIsEpmtyCollapseForRaveshErsal,
    saveAddress,
    // addressFormInputsVal,
    // setAddressFormInputsVal,
}: CollapseAndClickAwayForRaveshErsalCompType) => {
    console.log('050329-CollapseAndClickAwayForRaveshErsalComp rendered!!');   ////zare_nk_050329_added
    console.log('050401-CollapseAndClickAwayForRaveshErsalComp called!!-isEpmtyCollapseForRaveshErsal: ' + isEpmtyCollapseForRaveshErsal);
...
va jaigozine khoobi baraye neveshtane dastiye rooydade click dar useEffect hast) */}
        {/* <ClickAwayListener
            onClickAway={(event) => { 
                ...
                // const isToggleButton = target.id === 'openCollapseForRaveshErsalBtn';   ////zare_nk_050401_commented(chon age tage openCollapseForRaveshErsalBtn dakhelesh tage digehi mesle span ya img dashteh bashe va karbar 
                ////rooye oona click koneh barnameh target ro onna midoone na openCollapseForRaveshErsalBtn, chareye kar estefadeh az !!target.closest('#openCollapseForRaveshErsalBtn') hast)
                const isToggleButton = !!target.closest('#openCollapseForRaveshErsalBtn');   ////zare_nk_050401_added(amalgare !! meghdar ra be boolean tabdil mikoneh(null => false va HTMLElement=> true))

                console.log('050401-inja click shooodd!!!-isEpmtyCollapseForRaveshErsal: ' + isEpmtyCollapseForRaveshErsal + '-isToggleButton: ' + isToggleButton);
                if (!isEpmtyCollapseForRaveshErsal && !isToggleButton) {
                    console.log('050401-inja click shooodd!!!');
                    setIsEpmtyCollapseForRaveshErsal(true); // ببند
                }
            }}
        >
            <Collapse 
                ...
                style={{
                    ...
                    ...(!isEpmtyCollapseForRaveshErsal ? { zIndex: 1 } : { zIndex: 0 })
                }}
                in={!isEpmtyCollapseForRaveshErsal} //zare_nk_050202_nokteh(moadele show() va hide() dar bootstrap) 
                timeout="auto"
        ...
        <Drawer
            id="box"
            anchor="bottom"
            open={!isEpmtyCollapseForRaveshErsal}
            onClose={() => {
                console.log('zare_nk_050204-Drawer closed!');
                setIsEpmtyCollapseForRaveshErsal(true)
            }}
        ...
        >
            ...
            <form
                ...
                <div onClick={() => { radionClickedFunc(2) }}
                    style={{
                        display: "flex",
                        flexFlow: 'row',
                        width: '100%',
                ...
                <div onClick={() => { radionClickedFunc(3) }}
                    style={{
                        display: "flex",
                        flexFlow: 'row',
                        width: '100%',
...
export default memo(CollapseAndClickAwayForRaveshErsalComp);